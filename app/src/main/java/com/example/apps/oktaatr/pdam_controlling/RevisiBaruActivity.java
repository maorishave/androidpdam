...
import com.example.apps.oktaatr.pdam_controlling.Adapter.TRevisiAdapter;
...
public class RevisiBaruActivity extends AppCompatActivity {
    ...
    List<Bonc> boncList;
    TRevisiAdapter tRevisiAdapter = null;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_revisi_baru);
        boncList = new ArrayList<>();
        sessionManager = new SessionManager(getApplicationContext());
        dummy = findViewById(R.id.dummy);
        recyclerView = (RecyclerView) findViewById(R.id.listrevisibaru);
        btnback = (ImageButton) findViewById(R.id.btnback);
        btnback.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                Intent intent = new Intent(RevisiBaruActivity.this, RevisiFragment.class);
//                finish();
                onBackPressed();
            }
        });
    ...
    private void getListBonc() {
        ...
        Call<ListBoncModel> call = service.getBoncRBaru(sessionManager.getUserDetail().get(SessionManager.KEY_ID));
        call.enqueue(new Callback<ListBoncModel>() {
            ...
            public void onResponse(Call<ListBoncModel> call, Response<ListBoncModel> response) {
                if (response.body().getCode()==302){
                    ...
                    tRevisiAdapter = new TRevisiAdapter(getApplicationContext(), boncList);
                    recyclerView.setAdapter(tRevisiAdapter);
                    Toast.makeText(RevisiBaruActivity.this, response.body().getMessaage(), Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(RevisiBaruActivity.this, response.body().getMessaage(), Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<ListBoncModel> call, Throwable t) {
                Toast.makeText(RevisiBaruActivity.this, "error bro", Toast.LENGTH_SHORT).show();
            }
        });
